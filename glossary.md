...
| BM25    | Best Matching 25                                    | Keyword-search ranking function related to TF-IDF, with term-frequency saturation and document-length normalization.              | [note](ai/concepts/18_text_vectorization.md)              |
...
| Clustering | -                                                 | Unsupervised learning method that groups similar data points without pre-existing labels.                                         | [note](ai/concepts/19_clustering.md)                      |
...
| ERP     | Enterprise Resource Planning                        | Integrated software system managing core business processes: finance, HR, supply chain, and procurement.                          | [note](cloud/aws/devtools/04_sap_aws.md)                  |
| ETL     | Extract, Transform, Load                            | Data pipeline pattern: extract from source, transform or clean, then load into a target warehouse or lake.                        | -                                                         |
...
| k-means | k-means clustering                                  | Clustering algorithm that assigns points to the nearest of `k` centroids and repeatedly updates those centroids.                  | [note](ai/concepts/19_clustering.md)                      |
...
| MLOps   | Machine Learning Operations                         | Practice of automating and operationalizing ML workflows: training, evaluation, deployment, and monitoring.                       | -                                                         |
...
| RAG     | Retrieval-Augmented Generation                      | Pattern where an LLM retrieves relevant documents at query time and uses them as context before generating a response.            | [note](ai/concepts/16_rag.md)                             |
| RDF     | Resource Description Framework                      | W3C standard for representing knowledge as subject-predicate-object triples; backbone of the Semantic Web and ontologies.         | [note](cloud/aws/database/07_amazon_neptune.md)           |
...
| RRF     | Reciprocal Rank Fusion                              | Rank-fusion method that combines multiple search result lists by rewarding documents that rank highly in any list.                | [note](ai/concepts/16_rag.md)                             |
...
| TF-IDF  | Term Frequency-Inverse Document Frequency           | Text weighting method that gives high scores to terms frequent in one document but rare across the document collection.           | [note](ai/concepts/18_text_vectorization.md)              |
