---
description: Ingest a raw source file into the study repo wiki
argument-hint: <source-slug>
---

Run the ingest pipeline on the source identified by `$ARGUMENTS`.

Follow the full flow in [rules/09_ingest.md](../../rules/09_ingest.md):

1. Locate the source file in `raw/` (try `raw/$ARGUMENTS*` and `raw/**/$ARGUMENTS*`). If `$ARGUMENTS` is empty, ask the user which file to ingest.
2. Read the source.
