---
description: Sync structural documents after adding, moving, renaming, or deleting notes
argument-hint: <brief description of what changed>
---

Run the structural-document update flow defined in [rules/AGENTS.md §9](../../rules/AGENTS.md) and [rules/02_navigation.md §7](../../rules/02_navigation.md).

Trigger: any file was added, moved, renamed, or deleted. `$ARGUMENTS` should describe what changed (e.g., "added aws/compute/05_ecs.md").

Steps:

1. Identify the affected domain(s) from `$ARGUMENTS` or by asking the user if unclear.
...
5. For renames: run the sed pattern from [rules/07_scalability.md §8](../../rules/07_scalability.md) to fix all cross-references repo-wide.
