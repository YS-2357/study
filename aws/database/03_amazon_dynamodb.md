...
DynamoDB is often used as a status store for work items (jobs, orders, uploads) because it gives key-value lookups with single-digit millisecond performance.
...
### Conditional claim pattern

The important part is updating status safely so two workers do not claim the same job at once:
...
If the item is no longer `pending`, the update fails instead of silently letting two workers process the same job.

> **Tip:** If the workflow requires many joins, ad hoc reporting, or complex queries across many dimensions, DynamoDB may stop being a good fit. Keep the hot workflow state in DynamoDB and move reporting elsewhere.
...
---
← Previous: [Amazon Aurora](./02_amazon_aurora.md) | [Overview](./00_database_overview.md) | Next: [Amazon ElastiCache](./04_amazon_elasticache.md) →
