---
tags:
  -
created_at: 260417-141847
updated_at: 260417-141847
---

# service

AWS 201 service deep-dives. See [00_overview.md](00_overview.md) for study order.

- `README.md`: This file.
- `00_overview.md`: Study hub and navigation.
- `01_strands_agents_sdk.md`: Strands Agents SDK.
- `02_amazon_bedrock_agentcore.md`: Amazon Bedrock AgentCore.
- `agentcore/`: AgentCore service and capability notes.
- `03_amazon_bedrock_guardrails.md`: Amazon Bedrock Guardrails.
- `04_amazon_bedrock.md`: Amazon Bedrock.
- `05_amazon_bedrock_knowledge_bases.md`: Amazon Bedrock Knowledge Bases.
- `06_amazon_bedrock_agents.md`: Amazon Bedrock Agents.
- `07_aws_cdk.md`: AWS CDK.
- `08_amazon_cloudwatch.md`: Amazon CloudWatch.
- `09_aws_lambda.md`: AWS Lambda.
- `10_amazon_api_gateway.md`: Amazon API Gateway.
- `11_lambda_container_images.md`: Lambda container images.
- `12_mangum.md`: Mangum.
- `13_amazon_opensearch.md`: Amazon OpenSearch.
- `14_aws_ssm_parameter_store.md`: AWS SSM Parameter Store.
- `15_amazon_dynamodb_ttl.md`: DynamoDB TTL and session store.
- `16_amazon_s3.md`: Amazon S3 static frontend hosting.
- `17_amazon_cloudfront.md`: Amazon CloudFront.
- `18_amazon_ecr.md`: Amazon ECR.
- `19_amazon_dynamodb_workflow_status_tracking.md`: DynamoDB workflow status tracking.
- `20_amazon_bedrock_flows.md`: Amazon Bedrock Flows — visual deterministic pipeline builder.
- `21_amazon_bedrock_prompt_management.md`: Amazon Bedrock Prompt Management — versioned prompt store.
- `22_amazon_bedrock_model_evaluation.md`: Amazon Bedrock Model Evaluation — benchmark models on your dataset.
- `23_amazon_bedrock_data_automation.md`: Amazon Bedrock Data Automation — structured extraction from documents, images, video, audio.
- `24_amazon_bedrock_custom_models.md`: Amazon Bedrock Custom Models — fine-tuning and model import.
- `25_boto3.md`: boto3 — official AWS SDK for Python.
