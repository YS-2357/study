...
Amazon Bedrock Agents is a managed, low-code agent builder that orchestrates multi-step tasks by combining foundation models with action groups and [knowledge bases](05_amazon_bedrock_knowledge_bases.md) — configured through the console or API, no custom orchestration code required.
...
| **Model** | The foundation model that reasons and decides (via [Bedrock](04_amazon_bedrock.md)) |
| **Action groups** | Functions the agent can call — backed by Lambda, API schemas, or return-of-control |
| **Knowledge bases** | [RAG data sources](05_amazon_bedrock_knowledge_bases.md) the agent can query |
| **[Guardrails](03_amazon_bedrock_guardrails.md)** | Input/output filtering attached to the agent |
...
2. Attach a [knowledge base](05_amazon_bedrock_knowledge_bases.md) with your support articles
3. Create an action group `create-ticket` backed by a Lambda that writes to your ticketing system
4. Attach a [guardrail](03_amazon_bedrock_guardrails.md) to filter PII and block off-topic requests
...
Bedrock Agents is the fastest path to a working agent on AWS — no orchestration code, no infrastructure management. For teams that need custom orchestration logic or framework flexibility, [Strands Agents SDK](01_strands_agents_sdk.md) with [AgentCore](02_amazon_bedrock_agentcore.md) is the alternative. Understanding where each option fits prevents over-engineering simple use cases or under-engineering complex ones.
...
| Custom orchestration, full code control, any model | **[Strands Agents SDK](01_strands_agents_sdk.md)** |
| Production deployment at scale for any framework | **[AgentCore](02_amazon_bedrock_agentcore.md)** |
...
- For cross-session memory, integrate with [AgentCore Memory](02_amazon_bedrock_agentcore.md) or your own storage

### 4. Cost Layers
- Model inference tokens (per the [Bedrock](04_amazon_bedrock.md) model you choose)
...
---
← Previous: [Amazon Bedrock Knowledge Bases](05_amazon_bedrock_knowledge_bases.md) | [Overview](00_overview.md)
