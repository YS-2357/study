---
tags:
  - aws
created_at: 260417-141847
updated_at: 260417-141847
---

# AWS

AWS service notes. See [00_overview.md](00_overview.md) for study order.

## Files

- `README.md`: This file.
- `AGENTS.md`: AWS subtree agent rules.
- `CLAUDE.md`: AWS subtree Claude rules.
- `00_overview.md`: Study hub and navigation.

### Foundational (01-30)

- `01_region.md`: AWS Regions.
- `02_availability_zone.md`: Availability Zones.
- `03_subnet.md`: VPC subnets.
- `04_amazon_vpc.md`: Amazon VPC.
- `05_amazon_ec2.md`: Amazon EC2.
- `06_auto_scaling.md`: Auto Scaling.
- `07_aws_lambda.md`: AWS Lambda.
- `08_amazon_emr.md`: Amazon EMR.
- `09_amazon_kinesis.md`: Amazon Kinesis.
- `10_amazon_rds.md`: Amazon RDS.
- `11_amazon_aurora.md`: Amazon Aurora.
- `12_amazon_dynamodb.md`: Amazon DynamoDB.
- `13_amazon_elasticache.md`: Amazon ElastiCache.
- `14_security_group.md`: Security Groups.
- `15_amazon_iam.md`: Amazon IAM.
- `16_elastic_load_balancing.md`: Elastic Load Balancing.
- `17_aws_shield.md`: AWS Shield.
- `18_aws_waf.md`: AWS WAF.
- `19_amazon_s3.md`: Amazon S3.
- `20_amazon_ebs.md`: Amazon EBS.
- `21_amazon_cloudfront.md`: Amazon CloudFront.
- `22_amazon_efs.md`: Amazon EFS.
- `23_amazon_redshift.md`: Amazon Redshift.
- `24_amazon_sagemaker.md`: Amazon SageMaker.
- `25_aws_data_pipeline.md`: AWS Data Pipeline.
- `26_aws_fargate.md`: AWS Fargate.
- `27_pop.md`: Points of Presence.
- `28_cloud_computing_billing.md`: Cloud Computing Billing.
- `29_aws_marketplace.md`: AWS Marketplace.
- `30_aws_support_plans.md`: AWS Support Plans.

### Advanced (31-50)

- `31_strands_agents_sdk.md`: Strands Agents SDK.
- `32_amazon_bedrock_agentcore.md`: Amazon Bedrock AgentCore.
- `33_amazon_bedrock_guardrails.md`: Amazon Bedrock Guardrails.
- `34_amazon_bedrock.md`: Amazon Bedrock.
- `35_amazon_bedrock_knowledge_bases.md`: Amazon Bedrock Knowledge Bases.
- `36_amazon_bedrock_agents.md`: Amazon Bedrock Agents.
- `37_aws_cdk.md`: AWS CDK.
- `38_amazon_cloudwatch.md`: Amazon CloudWatch.
- `39_amazon_api_gateway.md`: Amazon API Gateway.
- `40_lambda_container_images.md`: Lambda Container Images.
- `41_mangum.md`: Mangum.
- `42_amazon_opensearch.md`: Amazon OpenSearch.
- `43_aws_ssm_parameter_store.md`: AWS SSM Parameter Store.
- `44_amazon_ecr.md`: Amazon ECR.
- `45_amazon_bedrock_flows.md`: Amazon Bedrock Flows.
- `46_amazon_bedrock_prompt_management.md`: Amazon Bedrock Prompt Management.
- `47_amazon_bedrock_model_evaluation.md`: Amazon Bedrock Model Evaluation.
- `48_amazon_bedrock_data_automation.md`: Amazon Bedrock Data Automation.
- `49_amazon_bedrock_custom_models.md`: Amazon Bedrock Custom Models.
- `50_boto3.md`: boto3.

## Subdirectories

- `agentcore/`: Deep-dive into Bedrock AgentCore components.
- `images/`: AWS Console screenshots.
