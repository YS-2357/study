...
Split an oversized or mixed-theme note into two focused notes.

**Argument:** `<note-path>`

Run the note-split workflow defined in [rules/07_scalability.md §3/§8](../07_scalability.md) and [rules/09_ingest.md §3.3](../09_ingest.md).

Split when the target note:
...
4. Run the sed rename pattern from [rules/07_scalability.md §8](../07_scalability.md) for each old filename replaced:
   ```bash
