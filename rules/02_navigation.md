...
- **`Related`**: explicit list of notes this one connects to conceptually. Derived from inline cross-references plus anything conceptually adjacent. Links here may duplicate inline links; that's expected.
...
A note with zero related links is valid but flagged by [10_lint.md](10_lint.md). Consider whether an inbound link from another note is missing.
...
**Numbering:** concept notes inside a subdomain are numbered per-subdomain (`01_*.md` to `NN_*.md`), not continuing from the parent.
...
3. **Renaming a note**: update all relative-path references (see [07_scalability.md](07_scalability.md) for the sed pattern).
