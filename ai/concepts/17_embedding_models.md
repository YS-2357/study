...
These outperform encoder-only models on MTEB because the larger pre-trained backbone carries more world knowledge. One key tweak: the causal attention mask is removed during fine-tuning so each token can attend to all others (bidirectional), not just left context.
...
At query time in a RAG pipeline: embed the user query → cosine-search the vector DB → retrieve top-K chunks → pass to LLM.
