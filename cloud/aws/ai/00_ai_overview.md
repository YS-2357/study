---
tags:
  - aws
  - ai
  - ml
created_at: 2026-04-18T12:30:09
updated_at: 2026-04-23T09:51:00
recent_editor: CLAUDE
---

# AWS AI Services — Overview

Amazon Bedrock, the Strands Agents SDK, and related generative-AI services. AgentCore deep-dive lives in the nested [agentcore/](agentcore/00_agentcore_overview.md) subtree.

## Concepts

- [Amazon Bedrock](01_amazon_bedrock.md) — Foundation model service.
- [Bedrock Guardrails](02_amazon_bedrock_guardrails.md) — Safety filters for AI.
- [Bedrock Knowledge Bases](03_amazon_bedrock_knowledge_bases.md) — RAG for foundation models.
- [Bedrock Agents](04_amazon_bedrock_agents.md) — AI agents with tool use.
- [Bedrock Flows](05_amazon_bedrock_flows.md) — Visual workflow builder for AI.
- [Bedrock Prompt Management](06_amazon_bedrock_prompt_management.md) — Version and manage prompts.
- [Bedrock Model Evaluation](07_amazon_bedrock_model_evaluation.md) — Benchmark and compare models.
- [Bedrock Data Automation](08_amazon_bedrock_data_automation.md) — Extract structured data from documents.
- [Bedrock Custom Models](09_amazon_bedrock_custom_models.md) — Fine-tune foundation models.
- [Amazon Bedrock AgentCore](10_amazon_bedrock_agentcore.md) — Managed agent runtime (summary; see [agentcore/](agentcore/00_agentcore_overview.md) for deep-dive).
- [Strands Agents SDK](11_strands_agents_sdk.md) — Open-source framework for building agents that deploy to AWS (AgentCore, Lambda, or self-hosted).
- [Bedrock Integration Patterns](12_amazon_bedrock_integration_patterns.md) — How the Bedrock services compose end-to-end.
- [LLM Evaluation](13_llm_evaluation.md) — Systematic methodology for measuring and comparing LLM quality.
- [Physical AI](14_physical_ai.md) — AI systems that perceive, reason about, and act in the physical world.
- [Agentic Modernization](15_agentic_modernization.md) — Using AWS agentic tools to modernize legacy applications.

## Subtree

- [AgentCore](agentcore/00_agentcore_overview.md) — Deep-dive into Bedrock AgentCore components.

---
↑ [AWS](../00_aws_overview.md)
